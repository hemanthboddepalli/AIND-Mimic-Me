...
// Update score being displayed
function setTimeSec(setTimeSec) {
  $("#timesec").html("Total Game Time: " + setTimeSec + " sec");
}
...
//
function game(canvas, img, face){
  // If the randomEmj is zero, call randomEmoji function to get the new Emoji.
  if(randomEmj === 0){
    randomEmoji();
  }

  // Get the unicode for the player face.
  unicd = toUnicode(face.emojis.dominantEmoji);
  // When the player emoji and the target emoji matches,
  if(unicd === randomEmj){
    // Prevent the function set with the setTimeout() to execute.
    clearTimeout(timeoutVar);
    // Alert the player that he has won a point,
    alert("Great!!! You have just won a point.");
    // Update the scores,
    gCorrect = gCorrect + 1;
    gTotal = gTotal + 1;
    setScore(gCorrect, gTotal);
    // And call randomEmoji to select the new Emoji.
    randomEmoji();
  }
}

// This function will be called to reset the score.
...
// Function to select the random Emoji, set this emoji and set the timeout.
function randomEmoji(){
  // Get the randomEmj unicode randomly.
  randomEmj = emojis[Math.floor(Math.random()*emojis.length)];
  // Set the Emoji to display it on HTML page.
  setTargetEmoji(randomEmj);
  // Set the timeout for 5 seconds.
  timeoutVar = setTimeout(resetGame, 5000);
  // Call the function to update the complete game time.
  setTimeSec(timeoutVar);
}

// This function will be called when the player unable to mimic.
function resetGame(){
  //Display the message
  alert("Sorry Buddy... You have just lost a point.");
  //Update the total but not correct.
  gTotal = gTotal + 1;
  setScore(gCorrect,gTotal);
  //Display another Emoji
  randomEmoji();
}
